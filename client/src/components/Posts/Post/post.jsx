...
export default function Post({ post }) {
    return (
        <div className="bg-[#11224A] rounded-xl shadow-lg p-6 flex flex-col gap-4 w-100 h-[500px] mx-auto">
            ...
            {post.selectedFile && (
                <div className="mb-2">
                    <img src={post.selectedFile} alt={post.title} className="w-full h-40 object-cover rounded-lg" />
                </div>
            )}
            ...
            <h2 className="text-xl font-semibold text-white mb-1 truncate">{post.title}</h2>
            <p className="text-gray-200 mb-2 overflow-hidden text-ellipsis" style={{maxHeight: '60px'}}>{post.message}</p>
